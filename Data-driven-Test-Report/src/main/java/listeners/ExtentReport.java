...
public class ExtentReport implements IReporter {
    ...
    public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
        ...
        try {
            FileInputStream = new FileInputStream(new File("src//main//resources//object.properties"));
    ...
    private void buildTestNodes(IResultMap tests, LogStatus status) throws Exception {
        ...
        if (tests.size() > 0) {
            for (ITestResult result : tests.getAllResults()) {
                test1 = extent1.startTest(((SheetColumnHeader) result.getParameters()[0]).getDescription());
